...
<!-- BEGIN:current-operating-baseline -->
# Current Operating Baseline

The July 2 nightly brief, `.claude/agents/CODEX-NIGHTLY-SESSION.agent.md`, and the
`codex/monthly-payments` branch instructions are historical records. They are
disabled and must not be used to start new work.

Before changing scope or behavior, read these sources in order:

1. `docs/REQUIREMENTS.md` for current capability status and accepted behavior.
2. `docs/ROADMAP.md` for active build order.
3. `docs/LINEAR-SYNC.md` and live Linear for current work status.
4. The latest dated handover only for evidence and implementation history.

Historical handovers and archived agent briefs never override current requirements,
live Linear status, owner decisions, or production behavior. New work starts from
`main` unless the current requirements name an active feature branch.
<!-- END:current-operating-baseline -->
